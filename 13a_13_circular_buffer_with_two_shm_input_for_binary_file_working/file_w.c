...
create_shm:
	...
	{	
		...
		numberOfBytesRead = read(fd, writeData, SIZE_TO_READ_FROM_FILE);
		...
		if(Free_Space_Present(pBufferMetadata) >= numberOfBytesRead)
		{
			//writing begins 
			Write_Into_SHM_Buffer(pBufferMetadata, writeData, numberOfBytesRead);
		}
